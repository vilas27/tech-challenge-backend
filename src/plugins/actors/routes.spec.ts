...
describe('plugin', () => describe('actor', () => {
  ...
  describe('POST /actors', () => {
    ...
    it('returns HTTP 201, with the `id` and `path` to the row created', async ({ context }: Flags) => {
      if (!isContext(context)) throw TypeError()
      const payload = { 'name': 'any-name', 'bio': 'repeated-bio', 'bornAt': new Date('1988-04-27') }
      const opts: Hapi.ServerInjectOptions = { method, url, payload }
      const anyResult = 123
  ...
  describe('PUT /actors/:id', () => {
    const paramId = 123
    const [method, url, payload] = ['PUT', `/actors/${paramId}`, { 'name': 'any-name', 'bio': 'repeated-bio', 'bornAt': new Date('1988-04-27') }]
