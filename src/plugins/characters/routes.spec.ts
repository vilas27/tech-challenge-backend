...
import * as lib from '../../lib/characters'
...
describe('plugin', () => describe('character', () => {
  ...
  describe('GET /characters', () => {
    const [method, url] = ['GET', '/characters']
  ...
  describe('POST /characters', () => {
    const [method, url] = ['POST', '/characters']
    ...
    it('returns HTTP 409 when given character (`name`, `movieId` and `actorId`) already exists', async ({ context }: Flags) => {
      if (!isContext(context)) throw TypeError()
      const payload = { 'name': 'repeated-name', 'movieId': 1, 'actorId': 1 }
      const opts: Hapi.ServerInjectOptions = { method, url, payload }
    ...
    it('returns HTTP 201, with the `id` and `path` to the row created', async ({ context }: Flags) => {
      if (!isContext(context)) throw TypeError()
      const payload = { 'name': 'any-name', 'movieId': 1, 'actorId': 1 }
      const opts: Hapi.ServerInjectOptions = { method, url, payload }
      const anyResult = 123
      ...
      sinon.assert.calledOnceWithExactly(context.stub.lib_create, payload.name, payload.movieId, payload.actorId)
      expect(response.result).equals({
        id: anyResult,
        path: `/characters/${anyResult}`
      })
    })
  ...
  describe('GET /characters/:id', () => {
    const paramId = 123
    const [method, url] = ['GET', `/characters/${paramId}`]
  ...
  describe('PUT /characters/:id', () => {
    const paramId = 123
    const [method, url, payload] = ['PUT', `/characters/${paramId}`, { 'name': 'any-name', 'movieId': 1, 'actorId': 1 }]
    ...
    it('returns HTTP 204', async ({ context }: Flags) => {
      ...
      sinon.assert.calledOnceWithExactly(context.stub.lib_update, paramId, payload.name, payload.movieId, payload.actorId)
  ...
  describe('DELETE /characters/:id', () => {
    const paramId = 123
    const [method, url] = ['DELETE', `/characters/${paramId}`]
