...
import { movie_and_characters as plugin } from './index'
import * as lib from '../../lib/movie_and_characters'

describe('plugin', () => describe('movie_and_characters', () => {
  ...
  describe('POST /movieAndCharacters', () => {
    const [method, url] = ['POST', '/movieAndCharacters']
    ...
    it('validates payload matches `movie_and_characters`', async ({ context }: Flags) => {
      if (!isContext(context)) throw TypeError()
    ...
    it('returns HTTP 201, with the `id` and `path` to the row created', async ({ context }: Flags) => {
      ...
      const payload = {
        'movie':
        {
          'name': 'any-name', 'synopsis': 'any-synopsis', 'releasedAt': '1988-04-27', 'runtime': 120, 'genreId': 1
        },
        'characters': [{
          'name': 'any-name', 'movieId': 1, 'actorId': 1
        }]
      }
      const opts: Hapi.ServerInjectOptions = { method, url, payload }
      const anyResult = 123
      ...
      expect(response.result).equals({
        id: anyResult,
        path: `/movieAndCharacters/${anyResult}`
      })
    })

  })

  describe('GET /movieAndCharacters/:id', () => {
    const paramId = 123
    const [method, url] = ['GET', `/movieAndCharacters/${paramId}`]
  ...
  describe('PUT /movieAndCharacters', () => {
    const [method, url, payload] = ['PUT', '/movieAndCharacters',{
      'movie':
      {
        'id':123, 'name': 'any-name', 'synopsis': 'any-synopsis', 'releasedAt': '1988-04-27', 'runtime': 120, 'genreId': 1
      },
      'characters': [{
        'id': 123, 'name': 'any-name', 'movieId': 1, 'actorId': 1
      }]
    }]
    ...
    it('returns HTTP 204', async ({ context }: Flags) => {
      if (!isContext(context)) throw TypeError()
      const opts: Hapi.ServerInjectOptions = { method, url, payload }
      context.stub.lib_update.resolves(123)

      const response = await context.server.inject(opts)
      expect(response.statusCode).equals(204)

      sinon.assert.calledOnceWithExactly(context.stub.lib_update, payload)
      expect(response.result).to.be.null()
    })

  })

  describe('DELETE /movieAndCharacters/:id', () => {
    const paramId = 123
    const [method, url] = ['DELETE', `/movieAndCharacters/${paramId}`]
