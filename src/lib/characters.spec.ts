...
import { list, find, findByMovieId, remove, create, update } from './characters'
import { knex } from '../util/knex'

describe('lib', () => describe('character', () => {
  ...
  describe('list', () => {
    ...
    it('returns rows from table `movie_character`', async ({context}: Flags) => {
      if(!isContext(context)) throw TypeError()
  ...
  describe('find', () => {
    ...
    it('returns one row from table `movie_character`, by `id`', async ({ context }: Flags) => {
      if (!isContext(context)) throw TypeError()
      const anyId = 1
  ...
  describe('findByMovieId', () => {
    ...
    it('returns rows from table `movie_character`, by `movieId`', async ({context}: Flags) => {
      if(!isContext(context)) throw TypeError()
      const anyMovieId = 123

      await findByMovieId(anyMovieId)
      sinon.assert.calledOnceWithExactly(context.stub.knex_from, 'movie_character')
      sinon.assert.calledOnceWithExactly(context.stub.knex_where, { movieId: anyMovieId })
    })
  ...
  describe('remove', () => {
    ...
    it('removes one row from table `movie_character`, by `id`', async ({context}: Flags) => {
      if(!isContext(context)) throw TypeError()
      const anyId = 123
  ...
  describe('update', () => {
    ...
    it('updates one row from table `movie_character`, by `id`', async ({context}: Flags) => {
      const anyId = 123
      if(!isContext(context)) throw TypeError()
      ...
      await update(anyId, anyName, anyMovieId, anyActorId)
      sinon.assert.calledOnceWithExactly(context.stub.knex_from, 'movie_character')
      sinon.assert.calledOnceWithExactly(context.stub.knex_where, { id: anyId })
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, { name: anyName, movieId: anyMovieId, actorId: anyActorId })
    })
    ...
    ; [0, 1].forEach( rows =>
      it(`returns ${!!rows} when (${rows}) row is found and deleted`, async ({context}: Flags) => {
        ...
        const result = await update(anyId, anyName, anyMovieId, anyActorId)
        expect(result).to.be.boolean()
        expect(result).equals(!!rows)
      }))
  ...
  describe('create', () => {
    ...
    it('insert one row into table `movie_character`', async ({context}: Flags) => {
      if(!isContext(context)) throw TypeError()
      const anyName = 'any-name'
      const anyMovieId = 1
      const anyActorId = 1
      ...
      await create(anyName, anyMovieId, anyActorId)
      sinon.assert.calledOnceWithExactly(context.stub.knex_into, 'movie_character')
      sinon.assert.calledOnceWithExactly(context.stub.knex_insert, { name: anyName, movieId: anyMovieId, actorId: anyActorId })
    })
    ...
    it('returns the `id` created for the new row', async ({context}: Flags) => {
      ...
      const result = await create(anyName, anyMovieId, anyActorId)
