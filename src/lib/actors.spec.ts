...
describe('lib', () => describe('actor', () => {
  ...
  describe('update', () => {
    ...
    it('updates one row from table `actor`, by `id`', async ({context}: Flags) => {
      ...
      const anyBornAt = new Date() // any date
  ...
  describe('create', () => {
    ...
    it('returns the `id` created for the new row', async ({context}: Flags) => {
      ...
      const anyId = 123
      const anyBio = 'any-bio'
      const anyBornAt = new Date('1988-04-27') // any date,
  ...
  describe('actorMovies', () => {

    it('returns list of movies for a specific actor', async ({context}: Flags) => {
      if(!isContext(context)) throw TypeError()
  ...
  describe('actorCharacters', () => {

    it('returns characters for a specific actor`', async ({context}: Flags) => {
      if(!isContext(context)) throw TypeError()
      const anyActorId= 123

      const result = await actorCharacters(anyActorId)
